...
fn build_scores_table(results: &str) -> HashMap<&str, Team> {
    ...
    for line in results.lines() {
        ...
        let v1 = scores.entry(team_1_name).or_insert(Team{goals_scored:0, goals_conceded:0});
        v1.goals_scored += team_1_score;
        v1.goals_conceded += team_2_score;
        let v2 = scores.entry(team_2_name).or_insert(Team{goals_scored:0, goals_conceded:0});
